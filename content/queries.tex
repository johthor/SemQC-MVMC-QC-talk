...
\begin{frame}{Solution for $\EX \qst_1\{p,q\}$}
  \begin{example}[Expand $\T_f( \qst_1\{p,q\})$]
    \begin{align*}
      \T_f( \qst_1\{p,q\}) =
        &(p \land q \land \uparrow\{p\land q\}) \lor \\
        &(p \land \lnot q \land \uparrow\{p\land \lnot q\}) \lor \\
        &(\lnot p \land q \land \uparrow\{\lnot p\land q\}) \lor \\
        &(\lnot p \land \lnot q \land \uparrow\{\lnot p\land \lnot q\})
    \end{align*}
  \end{example}
\end{frame}
...
\begin{frame}{Transform into Positive Query}
  Transform negative query $\phi[\qst_1]$ into $\phi[\qst_{1+},\qst_{1-}]$ denoted by $\myH$
  \begin{itemize}
    \item remove all negations for $\qst_{1}$
    ...
    \item postprocess solution for $\phi[\qst_{1+},\qst_{1-}]$
      \[
        \llbracket \phi(\qst_1) \rrbracket (s) = \{ a \mid (a,a) \in 
          \llbracket \myH (\phi)\phi[\qst_{1+},\qst_{1-}] \rrbracket (s) \}
      \]
  \end{itemize}
